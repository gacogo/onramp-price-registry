...
export const installContract = async (
  ...
  console.log(name, '(deez re-)installed as', bundleID.slice(0, 8));
  return installation;
};
...
export const startContract = async (
  ...
  console.log('deez STARTING CONTRACT', name);
  ...
  try{
    console.log('deeziiiiiiinstalling trying ', name);
    const installation = await consumeInstallation[name];
  console.log('installation succeeded and we now have this', installation);
  ...
  console.log('we got past startUpgradable, started is success, ', started);
  const { instance } = started;

  console.log('nayla instance is ', instance);
  produceInstance[name].reset();
  produceInstance[name].resolve(instance);

  console.log(name, 'deeziii started');

  if (!issuerNames) {
    console.log('NO ISSUER NAMES PROVIDED');
  ...
  console.log('deeziiiiiiissuerrrrs published');
  console.log('started.instance', started.instance);
  return started;
} catch (e) {
  console.log('nayla error is ', e);
}
};
